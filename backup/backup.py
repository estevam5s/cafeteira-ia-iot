...
def login_page():
    ...
    if token:
        try:
            data = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
            return redirect(url_for('coffee_system'))
        except:
            pass
...
def login():
    ...
    try:
        ...
        if user:
            ...
            response.set_cookie('token', token, httponly=True, secure=True if not app.debug else False)
            return response
...
# Sistema da Cafeteira (Protegido)
@app.route('/system')
@token_required
def coffee_system(current_user):
